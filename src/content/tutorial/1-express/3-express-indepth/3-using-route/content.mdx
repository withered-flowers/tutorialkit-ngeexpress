...
Pada code di atas, selain kita dapat mendefinisikan `endpoint` lebih dari satu yang sesuai dengan keinginan kita, kita juga dapat mendefinisikan `nested endpoint` seperti `localhost:3000/users/userId` di atas.

Namun code di atas, masih kurang benar, karena input `id` yang kita inginkan  tidak dapat dicetak pada browser. 

Output pada `localhost:3000/users/userId` di  browser adalah:
```
